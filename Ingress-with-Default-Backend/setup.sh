...
# nginx that serves "main-site" so functional tests can distinguish the backends.
kubectl apply -f - <<'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
...
# nginx that serves "error-page" so functional tests can distinguish the backends.
kubectl apply -f - <<'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
...
# NodePorts are reachable on any node's IP. We use the control-plane node (items[0]).
NODE_IP=$(kubectl get nodes \
  -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}')

# The baremetal manifest assigns a random NodePort in the 30000-32767 range.
# Read it from the live Service so it is always accurate.

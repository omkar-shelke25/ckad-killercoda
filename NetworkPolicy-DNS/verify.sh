...
kubectl get netpol "$np" -n "$ns" >/dev/null 2>&1 \
  || fail "NetworkPolicy '$np' not found in '$ns' (must be named exactly '$np')"
...
[[ "$eg_len" -eq 1 ]] || fail "egress should contain exactly 1 rule (found $eg_len)"
...
[[ "$ports_len" -eq 1 ]] || fail "egress rule should have exactly 1 port (found $ports_len)"

proto="$(echo "$spec_json" | jq -r '.spec.egress[0].ports[0].protocol // "UDP"')"
portv="$(echo "$spec_json" | jq -r '.spec.egress[0].ports[0].port')"
[[ "$proto" == "UDP" && "$portv" == "53" ]] || fail "egress port must be UDP/53 (found $proto/$portv)"
